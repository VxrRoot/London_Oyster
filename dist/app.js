...
const calculateTicketPrice = (traveledZonesToPay) => {
    let priceToPay = 0;
...
const setStationsInfo = (actualStation) => {
...
const startTravel = (station, type) => {
    ...
    ;
    if (!traveledZones.includes(zones[currentStation].zones))
        traveledZones.push(zones[currentStation].zones);
    if (type === 'next') {
        currentStation += 1;
        if (!traveledZones.includes(zones[currentStation].zones))
            traveledZones.push(zones[currentStation].zones);
        if (currentStation === zones.length - 1)
            alert("END STATION, Please leave the Train and start your travel again!");
    }
    else if (type === 'prev') {
        currentStation -= 1;
        if (!traveledZones.includes(zones[currentStation].zones))
